...
REWRITE_DEFN(TypeChecker, CallExpr, Expr, expr,) {
  ...
  MethodId mid = tinfo.methods.ResolveCall(&typeinfo_, curtype_, cc, TypeIdList(arg_tids), field_deref->FieldName(), field_deref->GetToken().pos, errors_);
...
REWRITE_DEFN(TypeChecker, NewClassExpr, Expr, expr,) {
  ...
  MethodId mid = tinfo.methods.ResolveCall(&typeinfo_, curtype_, cc, TypeIdList(arg_tids), tinfo.name, ref_type->Name().Tokens().back().pos, errors_);
  if (mid == kErrorMethodId) {
    return nullptr;
  }
...
REWRITE_DEFN(TypeChecker, FieldDerefExpr, Expr, expr,) {
  ...
  FieldId fid = tinfo.fields.ResolveAccess(&typeinfo_, curtype_, cc, expr.FieldName(), expr.GetToken().pos, errors_);
...
REWRITE_DEFN(TypeChecker, NameExpr, Expr, expr, exprptr) {
  ...
  ErrorList field_errors;
  {
    ...
    FieldId fid = tinfo.fields.ResolveAccess(&typeinfo_, curtype_, CallContext::INSTANCE, parts.at(0), toks.at(0).pos, &field_errors);
